async function initStats() {
  ...
  porCategoria.map((datos) => {
    ingresoYassitencia.push({
      ...
      ingreso: datos.data.map((item) =>
        item.assistance ? item.assistance * item.price : 0
      ),
      estimacionIngreso: datos.data.map((item) =>
        item.estimate ? item.estimate * item.price : 0
      ),
      asistencia: datos.data.map((item) =>
        item.assistance ? item.assistance : 0
      ),
      estimacionAsistencia: datos.data.map((item) =>
  ...
  ingresoYassitencia.forEach((categoria) => {
    ...
    categoria.porcentajeDeAsistencia =
      "%" + ((totalAsistencia * 100) / totalCapacidadPasados).toFixed(2);
    categoria.porcentajeDeEstimacion =
      "%" +
      ((totalAsistenciaEstimada * 100) / totalCapacidadFuturos).toFixed(2);
    ...
    categoria.estimacionIngreso.map(
      (ingresosEstimados) => (totalIngresoEstimado += ingresosEstimados)
    );
  ...
  //le agrego a cada evento una propiedad calculando el porcentaje de asistencia en funcion de la capacidad del evento
  ...
  tdMayorAsistencia.append(
    eventoMayorAsistencia[0].name +
      " %" +
      eventoMayorAsistencia[0].porcentajeAsistencia.toFixed(2)
  );

  rowMayoresYmenores.append(tdMenorAsistencia);
  tdMenorAsistencia.append(
    eventoMenorAsistencia[0].name +
      " %" +
      eventoMenorAsistencia[0].porcentajeAsistencia.toFixed(2)
  );

  rowMayoresYmenores.append(tdMayorCapacidad);
  tdMayorCapacidad.append(
    mayorCapacidad[0].name + "(" + mayorCapacidad[0].category + ")"
  );
